...
func generateBits(t *testing.T, size int) []Bit {
	t.Helper()
	ret := make([]Bit, size)
	for i := 0; i < size; i++ {
		ret[i] = 1
...
func TestGetBits(t *testing.T) {
	...
	cases := []testcase{
		{"from head", Offset{0, 0}, 4, []byte{0x0f}, generateBits(t, 4)},
		{"0011_1000", Offset{0, 3}, 3, []byte{0x38}, generateBits(t, 3)},
		{"0111_1000_0000_0000", Offset{1, 3}, 4, []byte{0x00, 0x78}, generateBits(t, 4)},
		{"0000_0011_1100_0000", Offset{0, 6}, 4, []byte{0xc0, 0x03}, generateBits(t, 4)},
		{"0111_1111_1100_0000", Offset{0, 6}, 9, []byte{0xc0, 0x7f}, generateBits(t, 9)},
		{"0111_1111_1111_1111_1100_0000", Offset{0, 6}, 17, []byte{0xc0, 0xff, 0x7f}, generateBits(t, 17)},
	}
